...
class ServiceRequestAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'short_id', 'customer_name', 'provider_name', 'category', 'region',
        'status_badge', 'is_urgent', 'preferred_date', 'final_price', 'created_at',
    )
    list_filter = ('status', 'is_urgent', 'category',
                   'region', 'preferred_date', 'created_at')
    search_fields = (
        'id', 'title',
    ...
    readonly_fields = (
        ...
        'customer_link', 'provider_link', 'cancellation_info',
    )
    add_fieldsets = (
        ('Parties', {
        ...
        ('Location & Scheduling', {
            'fields': ('region', 'address', 'latitude', 'longitude', 'preferred_date', 'preferred_time'),
        }),
        ('Pricing', {
    ...
    fieldsets = (
        ...
        ('Timestamps', {
            'fields': ('created_at', 'assigned_at', 'confirmed_at', 'started_at', 'completed_at', 'cancelled_at', 'updated_at'),
            'classes': ('collapse',),
        }),
    )
    actions = ['action_confirm', 'action_start',
               'action_complete', 'action_cancel_by_admin']
    ...
    def customer_link(self, obj):
        ...
        return format_html('<a href="{}">{} ({})</a>', url, obj.customer.get_full_name(), obj.customer.email)
    ...
    def provider_link(self, obj):
        ...
        return format_html('<a href="{}">{} ({})</a>', url, obj.provider.get_full_name(), obj.provider.email)
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('customer', 'provider', 'category', 'region')
    # Admin actions

    def _run_transition(self, request, queryset, transition_fn, past_tense, allowed_statuses):
        ...
        for obj in queryset:
            ...
            try:
                ...
                logger.error("Transition failed for %s: %s",
                             obj.pk, exc, exc_info=True)
                self.message_user(
        ...
        if skip_count:
            self.message_user(
                request, f"{skip_count} request(s) skipped (wrong status).", messages.WARNING)

    @admin.action(description="Mark as Confirmed")
    def action_confirm(self, request, queryset):
        self._run_transition(request, queryset, lambda obj: obj.confirm(), 'confirmed',
                             [ServiceRequestStatus.ASSIGNED])

    @admin.action(description="Mark as In Progress")
    def action_start(self, request, queryset):
        self._run_transition(request, queryset, lambda obj: obj.start(), 'started',
                             [ServiceRequestStatus.CONFIRMED])

    @admin.action(description="Mark as Completed")
    def action_complete(self, request, queryset):
        self._run_transition(request, queryset, lambda obj: obj.complete(), 'completed',
    ...
    def action_cancel_by_admin(self, request, queryset):
        self._run_transition(
            ...
            'cancelled',
            [ServiceRequestStatus.PENDING, ServiceRequestStatus.ASSIGNED,
             ServiceRequestStatus.CONFIRMED, ServiceRequestStatus.IN_PROGRESS],
        )
