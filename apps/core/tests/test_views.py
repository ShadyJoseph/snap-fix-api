...
class RegionListTests(APITestCase):
    ...
    def test_excludes_inactive_regions(self):
        create_region(name='Cairo',      slug='cairo',
                      code='CAI', is_active=True)
        create_region(name='Alexandria', slug='alexandria',
                      code='ALX', is_active=False)
        response = self.client.get(self.url)
        self.assertEqual(len(get_results(response)), 1)
    ...
    def test_location_returns_lat_lng(self):
        ...
        response = self.client.get(self.url)
        self.assertAlmostEqual(get_results(response)[
                               0]['latitude'],  30.0444, places=3)
        self.assertAlmostEqual(get_results(response)[
                               0]['longitude'], 31.2357, places=3)
